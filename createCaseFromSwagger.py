    ...
    def getSwaggerData(self, module, url_data):
        ...
        if not apis_path:
            ...
            try:
                ...
                new_data = requests.get(url, proxies=proxies, verify=False).json()

            data = diff2dict(old_data, new_data)
            if data.get('paths', None):
                #这里添加一个数据清洗步骤
            ...
            try:
                ...
                data = requests.get(url, proxies=proxies, verify=False).json()
            only_path = {}
            for key in apis_path:
                if not data.get('paths', None):
                    logger.error("出现了错误，暂计量下来：{}".format(data))
                    continue

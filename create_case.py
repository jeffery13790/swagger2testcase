...
common_config = {
    ...
    "validate": [{'eq': ['status_code', "${get_Status_code($status_code)}"]}, {'eq': ['body.code', "$body_code"]}, {'eq': ['body.msg', "$body_msg"]}, {'eq': ['headers."Content-Type"', "application/json;charset=UTF-8"]}],
...
if __name__ == "__main__":
    ...
    for module, url in url_dict.items():
        ...
        for request in testRequests:
            ...
            if not request.get('request', None):
                logger.error("这里出错了，错误信息{}".format(request))
                continue
